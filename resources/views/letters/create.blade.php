...
<body class="bg-gray-50 min-h-screen flex items-center justify-center py-12 px-6">
    ...
    <div class="bg-white p-8 md:p-12 w-full max-w-xl shadow-sm border border-gray-100">
        <div class="flex justify-between items-center mb-8">
            <h1 class="text-2xl serif-font italic">New Letter</h1>
            <a href="{{ route('letters.index') }}" class="text-xs uppercase text-gray-400 hover:text-black">Cancel</a>
        </div>

        <form action="{{ route('letters.store') }}" method="POST" class="space-y-6">
            @csrf

            <div>
                <label class="block text-xs uppercase tracking-widest text-gray-400 mb-2">Open When...</label>
                <input type="text" name="title" required placeholder="Contoh: Kamu merasa kangen..."
            ...
            <div>
                ...
                <textarea name="content" rows="8" required placeholder="Tulis pesan dari hati..."
                    class="w-full bg-gray-50 p-4 text-sm leading-relaxed border border-gray-100 rounded focus:outline-none focus:border-gray-400 focus:bg-white transition"></textarea>
            </div>
